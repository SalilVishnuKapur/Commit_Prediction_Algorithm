...
/*	 for(int io=start+1;io<start+num;io++)
  ...
  double xx,xxx,xxxx,U1_mid,U2_mid,U3_mid;
	  ...
	  double[][] multresult = {DecFuzz[i-1],DecFuzz[i-2],DecFuzz[i-3],DecFuzz[i-4]};
...
//   double[] u4=new double[start+num+5];
  ...
  // double[] u6=new double[start+num+5];
   double[] ForEnrl=new double[start+num+2]; 
   ForEnrl[i]=x7;
...
double[] finres ={result[3][0],result[3][1],result[3][2]};
      u1[i+1]=finres[0]; 
      u2[i+1]=finres[1];
      u3[i+1]=finres[2];
      //u4[i+1]=finres[3];
 	...
 	System.out.println(u1[i+1]+" "+u2[i+1]+" "+u3[i+1]);
 
 
 	if(u1[i+1]==0&&u2[i+1]==0&&u3[i+1]==0)
 	{
 		ForEnrl[i+1]=ForEnrl[i] + 0 ;
 	}
 	else
 	{
      double[] comp={u1[i+1],u2[i+1],u3[i+1]};
